...
def create_barplots(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者M3 - 三种情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('患者M3 - 关键AU三情绪对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(emotion_data, output_dir):
    ...
    plt.suptitle('患者M3 - 关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者M3 - 三种情绪AU激活热图', fontsize=14)
    ax.set_xlabel('情绪类型')
    ...
    ax.set_title('患者M3 - 悲伤vs积极AU差异', fontsize=12)
    plt.tight_layout()
    ...
    for emotion in ['悲伤', '中性', '积极']:
        ...
        ax.set_title(f'患者M3 - {emotion}情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_radar(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者M3 - 三种情绪AU轮廓雷达图', fontsize=14, pad=20)
...
def create_time_series(emotion_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'患者M3 - {au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者M3 - 三情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    plt.tight_layout()
...
4. 结论
-------
患者M3在三种情绪下表现出特定的AU激活模式。
注意：M3在积极和中性情绪下仍保持较高的AU04(皱眉)激活，
可能表现为习惯性皱眉或情感表达障碍。
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_患者M3_三种情绪对比')
    ...
    emotion_data, results_df = analyze_m3_cross_emotion()
