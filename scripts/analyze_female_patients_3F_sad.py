...
def analyze_patient_sad():
    print("="*70)
    print("女性患者3F悲伤情绪对比分析")
    print("="*70)
...
def create_barplots(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 悲伤情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('女性患者3F - 悲伤情绪关键AU对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(patient_data, output_dir):
    ...
    plt.suptitle('女性患者3F - 悲伤情绪关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 悲伤情绪AU激活热图', fontsize=14)
    ax.set_xlabel('患者')
    ...
    ax.set_title('女性患者3F - 悲伤情绪患者间AU差异热图', fontsize=14)
    plt.tight_layout()
    ...
    for pid in ['F1', 'F2', 'F3']:
        ...
        ax.set_title(f'女性患者{pid} - 悲伤情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_radar(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 悲伤情绪AU轮廓雷达图', fontsize=14, pad=20)
...
def create_time_series(patient_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'女性患者3F - 悲伤情绪{au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 悲伤情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    plt.tight_layout()
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_女患者3F_悲伤情绪对比')
    ...
    patient_data, results_df = analyze_patient_sad()
