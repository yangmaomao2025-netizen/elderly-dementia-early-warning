...
emotion_files = {
    ...
    '中性': '/root/.openclaw/media/inbound/file_33---0952895f-e350-4674-8b75-1c310d208392.csv',  # 不完整数据
...
def analyze_f1_cross_emotion():
    ...
    for emotion, filepath in emotion_files.items():
        try:
            ...
            frame_count = len(df)
            warning = ""
            if emotion == '中性' and frame_count < 1000:
                warning = " ⚠️ [数据不完整]"
                incomplete_warning = True
            print(f"{emotion}: {frame_count} frames{warning}")
        except Exception as e:
            print(f"Error loading {emotion}: {e}")
    
    if incomplete_warning:
        print("\n⚠️ 警告: F1中性情绪数据不完整，分析结果仅供参考")
    ...
    return emotion_data, results_df, incomplete_warning
...
def create_barplots(emotion_data, results_df, output_dir):
    ...
    width = 0.25
    colors = ['#FF6B6B', '#FFD93D', '#45B7D1']  # 中性用黄色标注
    ...
    ax.set_title('患者F1 - 三种情绪AU均值对比 (⚠️中性数据不完整)', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('患者F1 - 关键AU三情绪对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(emotion_data, output_dir):
    ...
    plt.suptitle('患者F1 - 关键AU分布箱线图 (⚠️中性数据不完整)', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F1 - 三种情绪AU激活热图 (⚠️中性数据不完整)', fontsize=14)
    ax.set_xlabel('情绪类型')
    ...
    ax.set_title('患者F1 - 悲伤vs积极AU差异', fontsize=12)
    plt.tight_layout()
    ...
    for emotion in ['悲伤', '中性', '积极']:
        ...
        title_suffix = " (⚠️数据不完整)" if emotion == '中性' else ""
        ax.set_title(f'患者F1 - {emotion}情绪AU激活{title_suffix}', fontsize=12)
        plt.tight_layout()
...
def create_radar(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F1 - 三种情绪AU轮廓雷达图 (⚠️中性数据不完整)', fontsize=14, pad=20)
...
def create_time_series(emotion_data, output_dir):
    ...
    for au in key_aus:
        ...
        for emotion in ['悲伤', '中性', '积极']:
            ...
            if emotion == '中性':
                label += ' (⚠️不完整)'
            ax.plot(time_sec, data_downsampled, label=label, alpha=0.7)
        
        ax.set_xlabel('时间 (秒)', fontsize=12)
        ax.set_ylabel(f'{au.replace("_r", "")} 激活强度', fontsize=12)
        ax.set_title(f'患者F1 - {au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def save_raw_data(emotion_data, output_dir):
    ...
    for emotion, df in emotion_data.items():
        ...
        suffix = '_INCOMPLETE' if emotion == '中性' and len(df) < 1000 else ''
        df[key_cols].to_csv(raw_dir / f'{emotion}{suffix}.csv', index=False, encoding='utf-8-sig')

def create_statistics(emotion_data, results_df, output_dir, incomplete_warning):
    ...
    if incomplete_warning:
        f_df['note'] = f_df.apply(lambda x: '中性数据不完整，结果仅供参考', axis=1)
    ...
    ax.set_title('患者F1 - 三情绪ANOVA F值 (⚠️中性数据不完整)', fontsize=14)
    plt.tight_layout()
    plt.savefig(stats_dir / 'statistics_anova_f_values.png', dpi=150)
    plt.close()

def generate_report(emotion_data, results_df, output_dir, incomplete_warning):
    ...
    warning_text = "\n⚠️ 重要警告：中性情绪数据不完整 (仅294帧，约10秒)\n         分析结果仅供参考，不建议用于正式结论\n" if incomplete_warning else ""
...
样本量:
  ...
  • 中性: {len(emotion_data['中性'])} 帧 ⚠️ 不完整数据
...
"""
    for emotion in ['悲伤', '中性', '积极']:
        warning = " ⚠️数据不完整" if emotion == '中性' else ""
        report += f"\n{emotion}:{warning}\n"
...
4. 结论
-------
患者F1在三种情绪下表现出特定的AU激活模式。
注意：由于中性情绪数据不完整，本分析结果仅供参考，
      不建议用于正式研究结论。
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_患者F1_三种情绪对比')
    ...
    emotion_data, results_df, incomplete_warning = analyze_f1_cross_emotion()
    ...
    print("生成统计分析...")
    create_statistics(emotion_data, results_df, output_dir, incomplete_warning)
    
    print("生成分析报告...")
    generate_report(emotion_data, results_df, output_dir, incomplete_warning)
