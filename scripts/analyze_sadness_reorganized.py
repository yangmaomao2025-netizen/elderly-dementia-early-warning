...
def create_subject_heatmap(data, stats, subject_name, output_path):
    ...
    ax.set_title(f'悲伤情绪 - {subject_name} AU激活模式', fontsize=14, pad=15)
...
def create_comparison_heatmap(results, output_path):
    ...
    ax.set_title('悲伤情绪 - 3被试AU激活对比', fontsize=14, pad=15)
...
def create_gender_diff_heatmap(results, output_path):
    ...
    ax.set_title('悲伤情绪 - 性别差异热力图', fontsize=14, pad=15)
...
def create_barplots(results, output_dir):
    ...
    ax.set_title('悲伤情绪 - 3被试AU均值对比', fontsize=14)
    ax.set_xticks(x)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('Mean Intensity', fontsize=12)
    ax.set_title('悲伤情绪 - 性别AU均值对比', fontsize=14)
    ax.set_xticks(x)
    ax.set_xticklabels([au.replace('_r', '') for au in KEY_AUS], fontsize=10)
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
...
def create_boxplots(results, output_dir):
    ...
    fig.suptitle('悲伤情绪 - 个体内AU分布箱线图', fontsize=14)
    plt.tight_layout()
...
def create_radar(results, output_dir):
    ...
    ax.set_title('悲伤情绪 - 个体AU轮廓雷达图', fontsize=14, pad=30)
...
def create_time_series(results, output_dir):
    ...
    for au in key_aus:
        ...
        fig.suptitle(f'悲伤情绪 - {au.replace("_r", "")} 时间序列', fontsize=14)
        plt.tight_layout()
...
def create_statistics(results, output_dir):
    ...
    ax.set_title('悲伤情绪 - AU差异对比', fontsize=14)
    ax.set_xticks(x)
...
def generate_report(results, output_dir):
    ...
    report.append("  - 悲伤_M1_male.csv")
    report.append("  - 悲伤_M2_male.csv")
    report.append("  - 悲伤_F1_female.csv")
    report.append("")
    report.append("=" * 80)
...
def main():
    print("=" * 70)
    print("悲伤情绪个体内差异与性别比较分析 - 参照上次标准")
    print("=" * 70)
    print()
    
    # 创建目录结构
    base_dir = '/root/.openclaw/workspace/analysis_results/2025-02-17_悲伤情绪_性别对比'
    ...
    for filename, info in SUBJECTS.items():
        ...
        dst = os.path.join(base_dir, f"raw_data/悲伤_{info['id']}_{info['gender']}.csv")
