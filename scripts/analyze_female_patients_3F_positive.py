...
# 女性患者积极情绪文件
patient_files = {
    'F1': '/root/.openclaw/media/inbound/file_36---182b42a2-87e5-41cf-af26-d505d000e246.csv',
    'F2': '/root/.openclaw/media/inbound/file_37---c15a0c41-ee72-464a-b765-40030db85592.csv',
    'F3': '/root/.openclaw/media/inbound/file_38---0e31b32a-8a78-4f18-9127-33ab67cbf863.csv'
}
...
def analyze_patient_positive():
    ...
    print("\n患者间差异分析:")
    print(f"F1 vs F2: {(results_df.loc['F1'] - results_df.loc['F2']).abs().mean():.3f}")
    print(f"F1 vs F3: {(results_df.loc['F1'] - results_df.loc['F3']).abs().mean():.3f}")
...
def create_barplots(patient_data, results_df, output_dir):
    ...
    for idx, pid in enumerate(['F1', 'F2', 'F3']):
    ...
    ax.set_title('女性患者3F - 积极情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('女性患者3F - 积极情绪关键AU对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(patient_data, output_dir):
    ...
    plt.suptitle('女性患者3F - 积极情绪关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 积极情绪AU激活热图', fontsize=14)
    ax.set_xlabel('患者')
    ...
    diff_matrix = pd.DataFrame(index=au_columns, columns=['F1-F2', 'F1-F3', 'F2-F3'])
    for au in au_columns:
        diff_matrix.loc[au, 'F1-F2'] = results_df.loc['F1', au] - results_df.loc['F2', au]
        diff_matrix.loc[au, 'F1-F3'] = results_df.loc['F1', au] - results_df.loc['F3', au]
    ...
    ax.set_title('女性患者3F - 积极情绪患者间AU差异热图', fontsize=14)
    plt.tight_layout()
    ...
    for pid in ['F1', 'F2', 'F3']:
        ...
        ax.set_title(f'女性患者{pid} - 积极情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_radar(patient_data, results_df, output_dir):
    ...
    ax.set_title('女性患者3F - 积极情绪AU轮廓雷达图', fontsize=14, pad=20)
...
def create_time_series(patient_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'女性患者3F - 积极情绪{au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(patient_data, results_df, output_dir):
    ...
    f_values = []
    for au in au_columns:
        f1_data = patient_data['F1'][au].values
        f2_data = patient_data['F2'][au].values
        f3_data = patient_data['F3'][au].values
        
        f_stat, p_val = stats.f_oneway(f1_data, f2_data, f3_data)
    ...
    ax.set_title('女性患者3F - 积极情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    plt.tight_layout()
...
1. 数据概况
-----------
分析对象: 女性抑郁症患者 (F1, F2, F3)
情绪类型: 积极情绪
数据来源: OpenFace 2.0 AU强度值

样本量:
  • F1: {len(patient_data['F1'])} 帧
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_女患者3F_积极情绪对比')
