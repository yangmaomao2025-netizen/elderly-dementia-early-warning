...
# -*- coding: utf-8 -*-
"""
F2数据预处理并生成热力图 (V5 - 68×45)
按要求：
- 宽：68行 (4单元×17AU)
- 高：45列 (3部分×15秒)
- 水平白线：分隔 F2_中性 | F2_积极前60秒 | F2_积极后60秒
- 垂直虚线：分隔4个单元
"""
...
def create_unit(au_sums, start_sec, end_sec):
    ...
    每个单元：17行(AU) × 15列(秒)
    """
    return au_sums.iloc[start_sec:end_sec].values.T  # 转置：17×15
...
def main():
    ...
    # 按列拼接3个部分：68×45
    # 顺序：F2_中性 | F2_积极前60秒 | F2_积极后60秒
    final_heatmap = np.hstack([neutral_heatmap, positive_first, positive_second])
    ...
    for sec in range(1, 16):
        col_labels.append(f"N_{sec}s")
    # F2_积极前60秒 (1-15秒)
    for sec in range(1, 16):
        col_labels.append(f"P1_{sec}s")
    # F2_积极后60秒 (1-15秒)
    for sec in range(1, 16):
        col_labels.append(f"P2_{sec}s")
    ...
    ax.set_title('F2 AU Heatmap (68 × 45)\nWidth: 68 rows (4 units × 17 AU) | Height: 45 cols (3 parts × 15s)', 
                 fontsize=14, fontweight='bold')
    ax.set_xlabel('Height: 45 cols (N | P1 | P2)', fontsize=12)
    ax.set_ylabel('Width: 68 rows (4 units × 17 AU)', fontsize=12)
    ...
    ax.text(22.5, 35, 'P1', ha='center', va='center', fontsize=12, color='black', fontweight='bold')
    ax.text(37.5, 35, 'P2', ha='center', va='center', fontsize=12, color='black', fontweight='bold')
    
    plt.tight_layout()
    
    # 保存
    output_path = os.path.join(output_dir, "F2_heatmap_68x45_v5.png")
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"\n✓ 热力图已保存: {output_path}")
    print(f"  尺寸: 68行 × 45列 (宽68, 高45)")
    
    # 保存数据
    data_path = os.path.join(output_dir, "F2_heatmap_data_68x45_v5.csv")
    pd.DataFrame(final_heatmap, index=row_labels, columns=col_labels).to_csv(data_path)
    print(f"✓ 数据已保存: {data_path}")
    
    print("\n" + "=" * 60)
    print("完成！热力图尺寸: 68行 × 45列 (宽68, 高45)")
    print("垂直白线分隔3部分 (N | P1 | P2)，水平虚线分隔4单元")
    print("=" * 60)
