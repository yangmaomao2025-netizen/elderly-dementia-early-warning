...
def create_barplots(emotion_data, results_df, output_dir):
    ...
    ax.set_title('M2对照组 - 三种情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('M2对照组 - 关键AU三情绪对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(emotion_data, output_dir):
    ...
    plt.suptitle('M2对照组 - 关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(emotion_data, results_df, output_dir):
    ...
    ax.set_title('M2对照组 - 悲伤vs积极AU差异', fontsize=12)
    plt.tight_layout()
    ...
    for emotion in ['悲伤', '中性', '积极']:
        ...
        ax.set_title(f'M2对照组 - {emotion}情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_time_series(emotion_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'M2对照组 - {au.replace("_r", "")} 时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(emotion_data, results_df, output_dir):
    ...
    ax.set_title('M2对照组 - 三情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    ax.axhline(y=stats.f.ppf(0.99, 2, 1000), color='red', linestyle='--', alpha=0.5, label='p<0.01阈值')
    plt.tight_layout()
...
def main():
    # 创建输出目录
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_M2_三种情绪对比')
