...
# 男性患者中性情绪文件
patient_files = {
    'M1': '/root/.openclaw/media/inbound/file_30---022dee2d-6990-4df3-aa41-a71b974bb24d.csv',
    'M2': '/root/.openclaw/media/inbound/file_31---b6f4948f-3181-49b5-8ea9-02070668994c.csv',
    'M3': '/root/.openclaw/media/inbound/file_32---5ceabcaa-84d6-4901-83aa-79b841590151.csv'
}
...
def create_barplots(patient_data, results_df, output_dir):
    ...
    ax.set_title('男性患者3M - 中性情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('男性患者3M - 中性情绪关键AU对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(patient_data, output_dir):
    ...
    plt.suptitle('男性患者3M - 中性情绪关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(patient_data, results_df, output_dir):
    ...
    ax.set_title('男性患者3M - 中性情绪AU激活热图', fontsize=14)
    ax.set_xlabel('患者')
    ...
    ax.set_title('男性患者3M - 中性情绪患者间AU差异热图', fontsize=14)
    plt.tight_layout()
    ...
    for pid in ['M1', 'M2', 'M3']:
        ...
        ax.set_title(f'男性患者{pid} - 中性情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_radar(patient_data, results_df, output_dir):
    ...
    ax.set_title('男性患者3M - 中性情绪AU轮廓雷达图', fontsize=14, pad=20)
...
def create_time_series(patient_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'男性患者3M - 中性情绪{au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(patient_data, results_df, output_dir):
    ...
    ax.set_title('男性患者3M - 中性情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    plt.tight_layout()
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_男患者3M_中性情绪对比')
