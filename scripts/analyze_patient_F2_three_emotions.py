...
def create_barplots(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F2 - 三种情绪AU均值对比', fontsize=14)
    ...
    ax.set_xlabel('Action Units', fontsize=12)
    ax.set_ylabel('激活强度', fontsize=12)
    ax.set_title('患者F2 - 关键AU三情绪对比', fontsize=14)
    ax.set_xticks(x + width)
...
def create_boxplots(emotion_data, output_dir):
    ...
    plt.suptitle('患者F2 - 关键AU分布箱线图', fontsize=16)
    plt.tight_layout()
...
def create_heatmaps(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F2 - 三种情绪AU激活热图', fontsize=14)
    ax.set_xlabel('情绪类型')
    ...
    ax.set_title('患者F2 - 悲伤vs积极AU差异', fontsize=12)
    plt.tight_layout()
    ...
    for emotion in ['悲伤', '中性', '积极']:
        ...
        ax.set_title(f'患者F2 - {emotion}情绪AU激活', fontsize=12)
        plt.tight_layout()
...
def create_radar(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F2 - 三种情绪AU轮廓雷达图', fontsize=14, pad=20)
...
def create_time_series(emotion_data, output_dir):
    ...
    for au in key_aus:
        ...
        ax.set_title(f'患者F2 - {au.replace("_r", "")}时间序列对比', fontsize=14)
        ax.legend()
        plt.tight_layout()
...
def create_statistics(emotion_data, results_df, output_dir):
    ...
    ax.set_title('患者F2 - 三情绪ANOVA F值 (红色:p<0.001, 橙色:p<0.01)', fontsize=14)
    plt.tight_layout()
...
def main():
    output_dir = Path('/root/.openclaw/workspace/老年失智人群预警模式科研项目/analysis_results/2026-02-18_患者F2_三种情绪对比')
    ...
    emotion_data, results_df = analyze_f2_cross_emotion()
